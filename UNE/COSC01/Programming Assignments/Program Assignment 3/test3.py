...
def create_widget(label2):
    try:
        ...
        my_button2 = Button(root, text="1", padx=25, borderwidth=4, command=lambda: rating(1, label2))
        my_button3 = Button(root, text="2", padx=25, borderwidth=4, command=lambda: rating(2, label2))
        my_button4 = Button(root, text="3", padx=25, borderwidth=4, command=lambda: rating(3, label2))
        my_button5 = Button(root, text="4", padx=25, borderwidth=4, command=lambda: rating(4, label2))
        my_button6 = Button(root, text="5", padx=25, borderwidth=4, command=lambda: rating(5, label2))
        ...
        label2.config(text=f"TclError: Widget Creation error - {widget_error}. Please restart the App and call the Administrator for support if the issue persists")
    except Exception as other_error:
        label2.config(text=f"Exception: Widget Creation error - {other_error}. Please restart the App and call the Administrator for support if the issue persists")
